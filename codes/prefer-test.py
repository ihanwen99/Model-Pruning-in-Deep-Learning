...
CLUSTERS_raw = json.load(open('prefer_cluster_filter_hanwen_1.json'))
print("hanwen-1-处理好了")
# 需要返回一个和原来一样的cluster
CLUSTERS = {}
for i in range(10):
    target = CLUSTERS_raw[str(i)]
    target.sort(key=lambda x: -x[1])
    newTarget = [i[0] for i in target]
    print(i, len(target), target)
    CLUSTERS[str(i)] = newTarget
...
def prune_hanwen(model_dict, curr):
    filters = curr
...
while len(curr) < left:
    for i in range(10):
        if choose[i] >= len(CLUSTERS[str(i)]): continue
        target = CLUSTERS[str(i)][choose[i]]
        if target not in curr:
            curr.append(target)
            choose[i] += 1
            if len(curr) >= left: break
        else:
            continue
...
print(num)
model = VGG19(num=num)
print("使用的是裁剪的模型，保留的神经元个数{}，保留的位置是{}".format(num, curr))
...
pretrained_dict = prune_hanwen(pretrained_dict, curr)
...
model.eval()
overall_acc = 0
torch.save(model, "model_left{}.pth".format(left))
print("model_left{}.pth".format(left))
import time

time0 = time.time()

# 这里也进行了修改！原来是10；同时修改了test loader
...
print("ALL Time is {}, Average Time is {}".format(temp, temp / 10000))
